...
Raycaster::Raycaster(int width, int height, char* window_name)
{
    ...
    SDL_Surface* txt2 = IMG_Load("../sprites/wall2.png");
     ...
     SDL_Surface* sprite1 = IMG_Load("../sprites/ring.png");
     SDL_Surface* sprite2 = IMG_Load("../sprites/ring.png");
     SDL_Surface* sprite3 = IMG_Load("../sprites/ring.png");
     SDL_Surface* sprite4 = IMG_Load("../sprites/ring.png");
...
void Raycaster::draw_sprite(tuple<double, double, SDL_Surface*> sprite)
{
    ...
    double sprite_size = (500.0/sprite_d) * 90.0;
    double sprite_x = (sprite_a - player[2])*1000.0/player[3] + 500.0 - sprite_size/2.0;
    ...
    for(auto x{sprite_x}; x < sprite_x + sprite_size; x++)
    {
        ...
        {
            if(x > 0 && x < 1000 && zBuffer[x] >= sprite_d)
            {
...
void Raycaster::render()
{
    ...
    {
        auto angle{player[2] - player[3]/2.0 + (player[3] * (double) i) / 1000.0};
     ...
     //Draws the minimap
    draw_rectangle(1.0, 1.0, 250.0, 250.0, {25.0, 25.0, 25.0});
    for(auto x{0}; x < 500; x+=block_size)
    {
        for(auto y{0}; y < 500; y+=block_size)
        {
            ...
            if(this->map[j][i] != ' ')
            {
                ...
                draw_rectangle(x/2, y/2, block_size/2, block_size/2);
            }
        }
    }
    ...
    for(auto enemy : enemies)
    {
        ...
        while (!quit) {
            ...
            while (SDL_PollEvent(&event)) 
            {
                ...
                if(player[0] > 205.0 && player[0] < 220.0 && player[1] > 395.0 && player[1] < 435.0)
                {
                    Mix_HaltMusic();
                ...
                if(event.type == SDL_KEYDOWN)
                {
                    ...
                    {
                        temp_x -= sin(player[2]) * 15;
                        temp_y += cos(player[2]) * 15;
                    }
                    ...
                    {
                        temp_x += cos(player[2]) * 15;
                        temp_y += sin(player[2]) * 15;
                    }
...
void Raycaster::win(string path, int font_size, string message, SDL_Color color, int x, int y)
{
    ...
    bool click{false};
    int current_button{0};
    SDL_Texture* background = IMG_LoadTexture(s, "../sprites/win_background.jpg");
    ...
    while (show)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            if(event.type == SDL_KEYDOWN)
            {
                ...
                player[1] = 70;
                player[2] = 0;
                render_main_menu("../fonts/font.ttf", 40, "Press any key to enter!", {255, 255, 255, 255}, 320, 200);
            }
        }
    }
}
